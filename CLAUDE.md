...
- 前後ナビゲーション
- ダークモード
- 404ページ（独自`src/pages/404.astro`。`disable404Route: true`でStarlight標準404ルートは無効化）
...
`src/components/PageTitle.astro`はStarlight標準PageTitleをラップし、status、実機・内容確認日、対象バージョン、対象コミット、platforms、tagsを表示する。
...
- 公開URLはカスタムドメイン`https://hermes.ai-deck.app`。`wrangler.jsonc`の`routes`で`custom_domain`紐付け済み。workers.dev URLは`workers_dev: false`で無効化済み（公開はカスタムドメインに一本化）。
...
- 非本番ブランチのプレビューは`wrangler.jsonc`の`preview_urls: true`で有効。workers.dev を再び有効化する場合は`workers_dev`を`true`に戻す。
...
- `vault/`全体（`.obsidian/`含む）はBiome対象外（`biome.json`の`files.ignore`に`vault`）。
