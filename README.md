...
このリポジトリは内容の種類ごとに異なる扱いを併用する。適用範囲と除外の正本は[LICENSING.md](./LICENSING.md)。

- **技術ドキュメント**（`vault/`配下の解説本文と図版）: [CC BY 4.0](./LICENSE)（クリエイティブ・コモンズ 表示 4.0 国際）
- **ソースコード**（`scripts/`、`src/`、設定ファイル）: [MIT License](./LICENSE-CODE)
- **著者・ブランド・集客に関する素材**（著者/サービス紹介、ロゴ・サイト名等のブランド要素、広告・アフィリエイト関連の記述）: 権利留保（All Rights Reserved。CC BY 4.0 の対象外）

「権利留保」は3つ目のライセンスではなく、ライセンスを付与しない既定の状態を指す。詳細は[LICENSING.md](./LICENSING.md)を参照。
