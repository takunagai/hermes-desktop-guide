...
> [!note] 出典と対象
> 本ページは公式ドキュメント `website/docs/reference/slash-commands.md`（`NousResearch/hermes-agent`、`main` @ `7eb9716`、2026-07-01 参照）に基づく。コマンド集合は正式リリース v0.17.0 後の `main` に追随する。プレフィックス一致（`/h`→`/help`、`/mod`→`/model`）に対応し、あいまいな場合はレジストリ順で最初の一致が優先される。
...
| `/goal <テキスト>` | ターンをまたいで追い続ける常設ゴールを設定（Ralph ループ的）。補助判定モデルが完了を判断し、未完なら自動継続。`/goal status\|pause\|resume\|clear`。既定20ターン |
...
| `/reasoning [level\|show\|hide]` | 推論の強度・表示を管理 |
...
| `/learn <学習対象>` | ディレクトリ・URL・直前の作業などから再利用可能なスキルを蒸留 |
