...
- [x] 補助タスクの UI 描画が8件（vision / web_extract / compression / skills_hub / approval / mcp / title_generation / curator）のままか確認する。→ 8件一致（2026-06-20 実機スクショ照合）
- [x] backend 専用スロットが `triage_specifier` / `kanban_decomposer` / `profile_describer` / `monitor` の4件で、いずれもモデル設定 UI に露出していないか確認する。→ 非露出を確認
- [x] Anthropic adaptive routing・modern thinking contract・`reasoning_effort`→verbosity・per-model `default_headers`・コンポーザーの per-model presets が設定画面に露出していないか確認する。露出していれば本ページへの追記を検討する。→ adaptive routing / thinking contract / per-model headers / composer presets は非露出。ただし新規の「Defaults / Reasoning」セレクタ（`agent.reasoning_effort`、v0.17 新規・ソース確定）が露出していたため「推論レベルのデフォルト」節を追記済み。
