...
- [ ] 補助タスクの UI 描画が8件（vision / web_extract / compression / skills_hub / approval / mcp / title_generation / curator）のままか確認する。
- [ ] backend 専用スロットが `triage_specifier` / `kanban_decomposer` / `profile_describer` / `monitor` の4件で、いずれもモデル設定 UI に露出していないか確認する。
- [ ] Anthropic adaptive routing・modern thinking contract・`reasoning_effort`→verbosity・per-model `default_headers`・コンポーザーの per-model presets が設定画面に露出していないか確認する。露出していれば本ページへの追記を検討する。
