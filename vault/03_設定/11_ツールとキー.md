...
| 6 | HERMES SIMPLEX TEXT BATCH DELAY | `HERMES_SIMPLEX_TEXT_BATCH_DELAY` | 未設定（実効: `0.8` 秒） | SimpleX で連続する短いテキストを1メッセージへまとめる静穏期間 | 既定のまま |
| 7 | RAFT PROFILE | `RAFT_PROFILE` | 未設定 | Raft エージェントのプロファイル slug。設定するとアダプタが有効化 | 使う場合だけ |
| 8 | SUDO PASSWORD | `SUDO_PASSWORD` | 未設定 | root 操作用 sudo パスワード | 保存非推奨 |

> [!note] v0.17 で削除された設定項目
> v0.16 の「設定」にあった `HERMES_TOOL_PROGRESS` / `HERMES_TOOL_PROGRESS_MODE` は v0.17 の設定サブメニューから削除された。ツール進捗は `display.tool_progress`（`config.yaml`、`off` / `new` / `all` / `verbose`）で管理する。env での後方互換フォールバックは残るが設定画面には表示されない。`tool_progress_grouping` も `config.yaml` の `display` 配下フィールドであり、環境変数カタログ（この画面）には現れない。
...
- `HERMES_SIMPLEX_TEXT_BATCH_DELAY`: SimpleX プラットフォームで、連続して届く短いテキストを1つのメッセージへまとめるまでの静穏期間（秒）。実効既定は `0.8`。自由入力。
- `RAFT_PROFILE`: Raft エージェントのプロファイル slug。設定するとアダプタが自動的に有効化される。製品既定はなし（未設定）。
...
> [!note] このページの確認対象
> 2026年6月19日公開の **Hermes Agent v0.17.0（リリースタグ `v2026.6.19`、コミット `2bd1977`）** を対象に、同タグの[環境変数カタログ](https://github.com/NousResearch/hermes-agent/blob/v2026.6.19/hermes_cli/config.py)、ツール設定、プロバイダープラグイン、[公式ドキュメント](https://hermes-agent.nousresearch.com/docs/user-guide/features/web-search)を照合し、ツール28項目・設定8項目を実機で確認した（2026-06-20）。[リリース情報](https://github.com/NousResearch/hermes-agent/releases/tag/v2026.6.19)

<!-- WAVE3-CHECK -->
## 実機確認チェックリスト（Wave 3 用）

- [x] 「ツール」サブメニュー28項目を v0.17 環境変数カタログで全数照合する（追加・削除・リネームの有無）→ 28項目・順序とも完全一致（2026-06-20 実機スクショ照合、追加/削除/改名なし）
- [x] 「設定」サブメニュー8項目を v0.17 環境変数カタログで全数照合する → 8項目だが構成変化。`HERMES_TOOL_PROGRESS`/`_MODE` 削除、`HERMES_SIMPLEX_TEXT_BATCH_DELAY`（既定 0.8秒）/ `RAFT_PROFILE`（既定なし）追加をソース確定し本文へ反映
- [x] 全数照合の結果に合わせて末尾の「このページの確認対象」注記（現状 v0.16.0 / タグ `v2026.6.5` / commit `3c231eb`）を v0.17 の版・タグ・コミットへ更新する → v0.17.0 / `v2026.6.19` / `2bd1977` へ更新
- [x] `tool_progress_grouping` が設定UI（環境変数カタログ）に**無い**ことを確認し、`HERMES_TOOL_PROGRESS` / `HERMES_TOOL_PROGRESS_MODE` の現行記述（移行互換入力・設定しない）が v0.17 でも妥当か裏取り → `tool_progress_grouping` は config.yaml の display フィールドで env カタログに無し（確定）。旧2項目は v0.17 で UI 削除のため記述撤去し「v0.17 で削除」note へ置換
