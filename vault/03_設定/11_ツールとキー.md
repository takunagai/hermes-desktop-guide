...
- [ ] 「ツール」サブメニュー28項目を v0.17 環境変数カタログで全数照合する（追加・削除・リネームの有無）
- [ ] 「設定」サブメニュー8項目を v0.17 環境変数カタログで全数照合する
- [ ] 全数照合の結果に合わせて末尾の「このページの確認対象」注記（現状 v0.16.0 / タグ `v2026.6.5` / commit `3c231eb`）を v0.17 の版・タグ・コミットへ更新する
- [ ] `tool_progress_grouping` が設定UI（環境変数カタログ）に**無い**ことを確認し、`HERMES_TOOL_PROGRESS` / `HERMES_TOOL_PROGRESS_MODE` の現行記述（移行互換入力・設定しない）が v0.17 でも妥当か裏取り
