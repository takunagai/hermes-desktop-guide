...
- [x] 接続モード（ローカル／リモート）と認証方式（OAuth／ユーザー名とパスワード／セッショントークン）の選択肢が実機と一致するか確認する。→ ローカル（既定）/ リモートの2択一致。リモートカード説明に OAuth / ユーザー名・パスワード / セッショントークンの自動判定記載を確認（2026-06-20 実機スクショ照合）。リモート URL 欄は常時表示。
- [x] gateway-to-gateway relay・signed-HTTP inbound 認証などのバックエンド機能が設定画面に露出していないことを確認する。露出していれば本ページへの追記を検討する。→ 非露出を確認（バックエンド機能は設定画面に出ない）
