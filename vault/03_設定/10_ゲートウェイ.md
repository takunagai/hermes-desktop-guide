...
- [ ] 接続モード（ローカル／リモート）と認証方式（OAuth／ユーザー名とパスワード／セッショントークン）の選択肢が実機と一致するか確認する。
- [ ] gateway-to-gateway relay・signed-HTTP inbound 認証などのバックエンド機能が設定画面に露出していないことを確認する。露出していれば本ページへの追記を検討する。
