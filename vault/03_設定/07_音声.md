...
| Kittentts | `kittentts` | いいえ | KittenTTS による音声合成 | 要実機確認（ローカル実行の可能性） |
| Piper | `piper` | いいえ | Piper による音声合成 | 要実機確認（ローカル実行の可能性） |

> [!note] v0.17 で追加された音声合成プロバイダー
> `xai` / `minimax` / `mistral` / `gemini` / `kittentts` / `piper` は v0.17 のプロバイダー一覧（enum）に含まれることを確認済み。ただし実機ドロップダウンへの表示、実行場所、必要な認証・前提は未採取。「必要なもの」欄の値は暫定で、Wave 3 で実機確認する。
...
> v0.17 では `stt.provider` の選択肢（enum）に `mistral` が含まれ、`stt.mistral.model` キーも存在する。以前の「Mistral は STT 選択肢に含まれない」という案内は v0.17 では当てはまらない。実機ドロップダウンへの表示と既定モデルは Wave 3 で確認する。
...
> [!note] 既定値は要実機確認（Wave 3）
> 次のキーは v0.17 のスキーマに存在することを確認済みだが、画面ラベルと既定値は実機で未採取。下表の値は暫定で、確定値ではない。Wave 3 で実機から採取して確定する。

| 画面表示（主・暫定） | 内部キー（サブ） | 製品既定値 | 入力・動作 |
|---|---|---|---|
| xAI 音声 ID | `tts.xai.voice_id` | 要実機確認 | xAI TTS の音声 ID |
| xAI 言語 | `tts.xai.language` | 要実機確認 | 読み上げ言語 |
| MiniMax モデル | `tts.minimax.model` | 要実機確認 | MiniMax TTS のモデル |
| MiniMax 音声 ID | `tts.minimax.voice_id` | 要実機確認 | MiniMax TTS の音声 ID |
| Mistral モデル | `tts.mistral.model` | 要実機確認 | Mistral TTS のモデル |
| Mistral 音声 ID | `tts.mistral.voice_id` | 要実機確認 | Mistral TTS の音声 ID |
| Gemini モデル | `tts.gemini.model` | 要実機確認 | Gemini TTS のモデル |
| Gemini 音声 | `tts.gemini.voice` | 要実機確認 | Gemini TTS の音声 |
| NeuTTS モデル | `tts.neutts.model` | 要実機確認 | NeuTTS のモデル |
| NeuTTS デバイス | `tts.neutts.device` | 要実機確認 | 実行デバイス（CPU / GPU など） |
| KittenTTS モデル | `tts.kittentts.model` | 要実機確認 | KittenTTS のモデル |
| KittenTTS 音声 | `tts.kittentts.voice` | 要実機確認 | KittenTTS の音声 |
| Piper 音声 | `tts.piper.voice` | 要実機確認 | Piper の音声 |
...
### v0.17 で追加された音声認識のサブキー

> [!note] 既定値は要実機確認（Wave 3）
> 次のキーは v0.17 のスキーマに存在することを確認済みだが、画面ラベルと既定値は実機で未採取。下表の値は暫定で、Wave 3 で確定する。

| 画面表示（主・暫定） | 内部キー（サブ） | 製品既定値 | 入力・動作 |
|---|---|---|---|
| Mistral STT モデル | `stt.mistral.model` | 要実機確認 | Mistral 文字起こしのモデル |
| Groq STT モデル | `stt.groq.model` | 要実機確認 | Groq 文字起こしのモデル |
...
- [ ] TTS の新プロバイダー（xai / minimax / mistral / gemini / kittentts / piper）が実機のドロップダウンに表示されるか確認する。
- [ ] STT の `mistral` が実機のドロップダウンに表示されるか確認する。
- [ ] 新規 TTS サブキー（`tts.xai.*` / `tts.minimax.*` / `tts.mistral.*` / `tts.gemini.*` / `tts.neutts.*` / `tts.kittentts.*` / `tts.piper.voice`）の画面ラベルと既定値を採取し、暫定表を確定値へ更新する。
- [ ] 新規 STT サブキー（`stt.mistral.model` / `stt.groq.model`）の画面ラベルと既定値を採取する。
- [ ] kittentts / piper / neutts の実行場所（ローカル／クラウド）と必要な前提（モデル取得・計算資源・API 認証）を確定し、プロバイダー表の「必要なもの」欄を更新する。
