...
> [!note] v0.17 で追加（既定値は要実機確認）
> Docker / Singularity / Modal / Daytona の各実行バックエンドに、使用するコンテナ／環境イメージを指定する欄（`terminal.docker_image`・`terminal.singularity_image`・`terminal.modal_image`・`terminal.daytona_image`）が追加された。既定イメージ名は未確認（Wave 3 実機確認待ち。根拠: v0.17 設定UI定義 `constants.ts` @ `2bd1977`、v0.16 との比較。参照日 2026-06-20）。
...
- [ ] backend-image 4欄（`terminal.docker_image`「Docker イメージ」/ `terminal.singularity_image`「Singularity イメージ」/ `terminal.modal_image`「Modal イメージ」/ `terminal.daytona_image`「Daytona イメージ」）が設定画面に実在し、ラベル文言が一致するか
- [ ] backend-image 4欄の **既定イメージ名** を実機で採取し、上の needs-review コールアウトを確定値へ更新する
- [ ] watch-windows が設定画面に項目として存在しないことを確認（存在しなければ Wave 4 変更履歴側で扱う）
- [ ] background subagents（`delegate_task(background=true)` 相当）が設定画面に項目として存在しないことを確認
- [ ] `terminal.backend` の enum（`local` / `docker` / `singularity` / `modal` / `daytona` / `ssh`）が実機の選択肢と一致するか
