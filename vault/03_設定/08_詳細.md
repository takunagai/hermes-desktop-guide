...
- [x] backend-image 4欄（`terminal.docker_image`「Docker イメージ」/ `terminal.singularity_image`「Singularity イメージ」/ `terminal.modal_image`「Modal イメージ」/ `terminal.daytona_image`「Daytona イメージ」）が設定画面に実在し、ラベル文言が一致するか → 4欄とも実在・ラベル一致（常時表示、2026-06-20 実機スクショ照合）
- [x] backend-image 4欄の **既定イメージ名** を実機で採取し、上の needs-review コールアウトを確定値へ更新する → Docker/Modal/Daytona=`nikolaik/python-nodejs:python3.11-nodejs20`、Singularity=`docker://nikolaik/python-nodejs:python3.11-nodejs20` を採取し確定値へ更新
- [x] watch-windows が設定画面に項目として存在しないことを確認（存在しなければ Wave 4 変更履歴側で扱う）→ 非露出を確認（Wave 4 送り）
- [x] background subagents（`delegate_task(background=true)` 相当）が設定画面に項目として存在しないことを確認 → 非露出を確認（Wave 4 送り）
- [x] `terminal.backend` の enum（`local` / `docker` / `singularity` / `modal` / `daytona` / `ssh`）が実機の選択肢と一致するか → 6種一致（実機並び順 Local/Docker/Singularity/Modal/Daytona/Ssh に表を揃えた）。なお最大エージェントステップは実機 60 表示だが製品既定 90（ソース確定）でユーザー変更と判断
